            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex flex-wrap items-center mb-6 space-y-4 md:space-y-0 md:space-x-4">
                        <!-- Search Bar -->
                        <form action="{{ route('alumni.search') }}" method="GET" class="flex-grow relative w-full md:w-auto">
                            <input
                                type="text"
                                name="search"
                    ...
                    <form action="{{ route('alumni.showEmployabilityTracerData') }}" method="GET" class="space-y-4 md:space-y-0 md:flex md:flex-wrap md:items-center md:space-x-4 mb-6">
                        <!-- Filter Dropdown -->
                        <div class="flex-shrink-0 w-full md:w-1/4">
                            <label for="course" class="block text-sm font-medium text-gray-700 mb-1">Course</label>
                            <select id="course" name="course" onchange="this.form.submit()" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-red-400">
                                <option value="all">All Courses</option>
                                @foreach($courses as $course)
                                    <option value="{{ $course }}" {{ request()->input('course') == $course ? 'selected' : '' }}>{{ $course }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="flex-shrink-0 w-full md:w-64">
                            <label for="sort_last_name" class="block text-sm font-medium text-gray-700 mb-1">Sort by Last Name</label>
                            <select id="sort_last_name" name="sort_last_name" onchange="this.form.submit()" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-red-400">
                                <option value="">Select</option>
                                <option value="asc" {{ request()->input('sort_last_name') == 'asc' ? 'selected' : '' }}>Ascending</option>
                                <option value="desc" {{ request()->input('sort_last_name') == 'desc' ? 'selected' : '' }}>Descending</option>
                            </select>
                        </div>
                        ...
                        <div class="flex-shrink-0 w-full md:w-1/4">
                            ...
                            <select id="employment_status" name="employment_status" onchange="this.form.submit()" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-red-400">
                                <option value="all">All Statuses</option>
                                @foreach($employmentStatuses as $status)
                                    <option value="{{ $status }}" {{ request()->input('employment_status') == $status ? 'selected' : '' }}>{{ $status }}</option>
                                @endforeach
                            </select>
                        </div>
                    </form>
                    ...
                    <div class="overflow-x-auto">
                        <div class="table-container max-h-[calc(100vh-16rem)] overflow-y-auto">
                            <table id="employabilityTable" class="min-w-full table-auto border-collapse border border-gray-300">
                                <thead>
                                    <tr class="bg-gray-200">
                                        ...
                                        <th class="border px-2 py-2 text-xs md:text-sm">Status of Employment prior to graduation</th>
                                        <th class="border px-2 py-2 text-xs md:text-sm">Status of Employment after graduation</th>
                                ...
                                <tbody>
                                    @foreach($alumniData as $index => $data)
                                        <tr class="bg-white border-b">
                                            <td class="border px-2 py-2 text-center text-xs md:text-sm">{{ $loop->iteration }}</td>
                                            <td class="border px-2 py-2 text-xs md:text-sm">{{ $data->courseGraduated->course_name ?? 'N/A' }}</td>
                                            <td class="border px-2 py-2 text-xs md:text-sm">{{ $data->last_name }}, {{ $data->first_name }}</td>
                                            <td class="border px-2 py-2 text-xs md:text-sm">{{ $data->year_graduated ?? 'N/A' }}</td>
                                            <td class="border px-2 py-2 text-xs md:text-sm">{{ $data->professionalData->inclusive_from ?? 'N/A' }}</td>
                                            <td class="border px-2 py-2 text-xs md:text-sm"></td>
                                            <td class="border px-2 py-2 text-xs md:text-sm">{{ $data->professionalData->employmentStatus->status_name ?? 'N/A' }}</td>
                                            <td class="border px-2 py-2 text-xs md:text-sm">{{ $data->professionalData->present_position ?? 'N/A' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
